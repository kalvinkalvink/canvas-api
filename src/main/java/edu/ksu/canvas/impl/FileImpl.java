...
public class FileImpl extends BaseImpl<File, FileReader, FileWriter> implements FileReader, FileWriter {
	...
	@Override
	public List<File> listCourseFile(ListCourseFileOptions listCourseFileOptions) throws IOException {
		String url = buildCanvasUrl("courses/" + listCourseFileOptions.getCourseId() + "/files", listCourseFileOptions.getOptionsMap());
		return getListFromCanvas(url);
	}

	@Override
	protected Type listType() {
		return new TypeToken<List<File>>() {
		}.getType();
	}
